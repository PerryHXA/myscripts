...
AddEventHandler('Oku_Laskutus:sendBill', function(playerId, sharedAccountName, label, amount)
	...
	if amount > 0 and xTarget then
		TriggerEvent('Oku_Accounts:getSharedAccount', sharedAccountName, function(account)
			if account then
...
VorpCore.addRpcCallback('Oku_Laskutus:getBills', function(source, cb) 
	local identifier = GetPlayerIdentifiers(source)[1]
...
VorpCore.addRpcCallback('Oku_Laskutus:getTargetBills', function(source, cb, target) 
...
VorpCore.addRpcCallback('Oku_Laskutus:payBill', function(source, cb, billId) 
	...
	MySQL.Async.fetchAll('SELECT sender, target_type, target, amount FROM billing WHERE id = @id', {
		...
		if result[1] then
			...
			if result[1].target_type == 'society' then
				TriggerEvent('Oku_Accounts:getSharedAccount', result[1].target, function(account)
